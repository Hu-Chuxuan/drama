...
def process_output(content, input_file, sources, task, save_dir, search_query):
    filename = f"{save_dir}/{input_file['id']}.json"
    thought_match = re.search(r"Thought:\s*(.*?)\s*Action:", content, re.DOTALL)
    thought_text = thought_match.group(1).strip() if thought_match else ""
    action_match = re.search(r"Action:\s*(.*?)```csv", content, re.DOTALL)
    action_text = action_match.group(1).strip() if action_match else ""
    csv_match = re.search(r"```csv\s*(.*?)\s*```", content, re.DOTALL)
    csv_text = csv_match.group(1).strip() if csv_match else ""
    py_match = re.search(r"```python\s*(.*?)\s*```", content, re.DOTALL)
    py_text = py_match.group(1).strip() if py_match else ""
    ...
    if task == "qa":
        ...
        exec_match = re.search(r"```answer\s*(.*?)\s*```", content, re.DOTALL)
        result_text = exec_match.group(1).strip() if exec_match else ""
        result_text = True if result_text.lower() == "true" else False if result_text.lower() == "false" else result_text

    output_dict = {
        "id":    input_file["id"],
        "claim": input_file["ques"],
        "label": input_file["label"],
        "searched query": search_query,
...
def plan_search_term(query, task):
...
def get_web_element_rect(browser, fix_color=True):
    ...
    js_script = """
        (function removeContent() {{
            const aiOverviewEls = document.querySelectorAll('div[data-mcpr], div.AQisZb');
            aiOverviewEls.forEach(el => el.remove());

            const summaryEls = document.querySelectorAll('div[data-md="471"], div.Wt5Tfe');
            summaryEls.forEach(el => el.remove());

            const snippetEls = document.querySelectorAll('div.VwiC3b, div.P1usbc');
            snippetEls.forEach(el => el.remove());

            document.querySelectorAll('h3.LC20lb.MBeuO.DKV0Md').forEach(el => {
...
def eval_result(id, result, claim, save_dir):
    data = {}
    data["id"] = id
    data["claim"] = claim
    data["result"] = result
